...
require "teien/dispatcher"
require "teien/server_network"
require "teien/client_network"
    ...
    EM.run do
      EM.add_periodic_timer(tick_period) do
        if @last_time == 0
          ...
          if @quit
            EM.stop 
            Teien::get_component("garden").finalize()
          end
        end
      end

      Signal.trap("INT")  { EM.stop; Teien::get_component("garden").finalize() }
      Signal.trap("TERM") { EM.stop; Teien::get_component("garden").finalize() }
